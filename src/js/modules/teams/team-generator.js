...
export function generateTeams(eleves, options) {
    ...
    if (options.mixite === 'non-mixte') {
        ...
        // Le mode "niveau" (hétérogène) répartit par blocs, mais on garde le pool mixte entrelacé
        // pour éviter les équipes non mixtes.
        teams = buildTeams(poolMixte, options, 0, false);
    } 
    else {
        // Mode 'ignore'
...
function buildTeams(pool, options, startIndex, doSort) {
    ...
    if (options.mode === 'niveau') {
        ...
        // Pour garantir la mixité, on mélange les sexes à l'intérieur de chaque bloc
