...
export function renderBadmintonLive() {
    ...
    // 1) Lire la config pour connaître le mode (terrain / maniere)
    let configMode = 'frontback';
    onValue(configRef, (snap) => {
        const cfg = snap.val() || {};
        configMode = cfg.mode || 'frontback';
        renderBadmintonLiveContent(configMode); // re-render dès que la config change
    }, { onlyOnce: false });
    ...
    function renderBadmintonLiveContent(mode) {
        ...
        modeUnsub = onValue(resultsRef, async (snap) => {
            ...
            for (const m of matchs.slice(0, 20)) {
                ...
                // ─── Ligne "manière" uniquement si mode = maniere ───
                let footerHtml = '';
                if (isManiere) {
                    ...
                    // Mode terrain : on affiche les stats de zone (si présentes)
    ...
    // Écoute config une seule fois pour init
    let modeUnsub = null;
