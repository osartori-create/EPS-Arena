...
/**
 * Affiche un radar des 7 aptitudes pour un élève (version Canvas)
 */
export function afficherRadar(container, data, eleveId) {
    ...
    if (typeof Chart === 'undefined') {
        ...
        script.onload = () => renderRadarChart('eval-radar-canvas', labels, valeurs);
        document.head.appendChild(script);
    } else {
        renderRadarChart('eval-radar-canvas', labels, valeurs);
    }
}

/**
 * Render le radar pour la fiche élève (appelé depuis la fiche)
 */
export function renderRadar(canvas, data, eleveId) {
    ...
    // Si le canvas est un élément HTML, on récupère son ID
    const canvasId = typeof canvas === 'string' ? canvas : canvas.id;
    renderRadarChart(canvasId, labels, valeurs);
}

/**
 * Fonction interne pour dessiner le radar avec Chart.js
 */
function renderRadarChart(canvasId, labels, valeurs) {
...
export function afficherPodium(container, data, testId) {
    ...
    const avecResultats = eleves
        ...
        .filter(e => e.resultat !== null && e.resultat.groupe !== null);
    
    if (avecResultats.length === 0) {
        container.innerHTML = '<p class="text-slate-400 text-center">Aucun résultat pour ce test.</p>';
        return;
    }
...
/**
 * Récupère la valeur pour le podium
 */
