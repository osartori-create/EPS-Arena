// src/js/ui/prof/tv-ui.js
import { getConfigData, getEscaladeData } from '../../core/live-engine.js';

export function initTVUI() {
    // On écoute les mises à jour des données pour l'onglet TV
    window.addEventListener('live-data-updated', (e) => {
        const { type, data } = e.detail;
        if (type === 'escalade') renderTV();
    });
}
...
function renderTV() {
    ...
    Object.keys(config).forEach(key => {
        if (key !== 'activite' && (typeof config[key] === 'number' || Array.isArray(config[key]))) {
            let count = typeof config[key] === 'number' ? config[key] : config[key].length;
            equipes.push({ lettre: key, count: count, score: 0 });
        }
    });

    // 2. Calculer le total de points par équipe (somme des mètres de tous les membres)
    ...
    // Les niveaux de la montagne (bandes)
    ...
    // Les grimpeurs (équipes)
    html += `<div class="relative z-10 flex justify-around items-end pb-4">`;
    equipes.forEach(eq => {
        const height = Math.max((eq.score / maxScore) * 90, 5); // Minimum 5% pour rester visible
        html += `
