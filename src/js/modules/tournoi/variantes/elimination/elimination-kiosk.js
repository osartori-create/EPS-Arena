    ...
    container.innerHTML = `
        <div class="bg-slate-800 p-6 rounded-2xl border border-slate-700 text-center max-w-md mx-auto">
            ...
            <div class="mb-6">
                ...
                <input type="number" id="tournoi-code-input" class="w-full bg-slate-900 border-2 border-slate-600 rounded-xl p-4 text-center text-4xl font-black text-white" placeholder="1" min="1" max="99" value="${currentCode || ''}" oninput="window.tournoiSetCode(this.value)">
            </div>
            <button id="tournoi-btn-elimine" class="w-full bg-red-600 py-6 rounded-3xl text-3xl font-black uppercase shadow-xl active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed" onclick="window.tournoiElimine()" disabled>❌ Je suis éliminé</button>
