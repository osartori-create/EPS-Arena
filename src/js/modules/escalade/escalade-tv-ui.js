...
export async function renderEscaladeTV() {
    ...
    currentUnsub = onValue(monteesRef, async (snap) => {
        ...
        // Récupérer les groupes depuis la config
        ...
        // Récupérer le mapping local pour les photos
        const localMapping = getLocalMapping(classe) || {};
        const eleves = JSON.parse(localStorage.getItem(`eps_arena_eleves_${classe}`) || '[]');

        // Construire les scores par groupe
        const equipes = [];
        ...
        for (const eq of equipesAvecScore) {
            ...
            // Récupérer les photos des membres du groupe
